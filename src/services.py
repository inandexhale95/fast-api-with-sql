...
def get_user_list(db: _orm.Session, skip: int, limit: int):
...
def get_post_list(db: _orm.Session, skip: int, limit: int):
    return db.query(_models.Post).offset(skip).limit(limit).all()


def get_post_info(db: _orm.Session, post_id: int):
...
def update_post(db: _orm.Session, post: _schemas.PostCreate, post_id: int):
    db_post = get_post_info(db=db, post_id=post_id)
